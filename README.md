...
| `apps/browser-client/src/attachments` | Adjacent note files and device cache | Attachment catalog, verified download cache, and responsive shelf | Cache keys, object URLs, transfer progress, desktop rail, and mobile bottom sheet |
...
Signed-in users can attach up to 20 files of 25 MB each to a note while connected. Files sit beside the note in a desktop rail or a mobile bottom sheet and never become part of its Markdown. Opening a file verifies and caches it on that device; cached files remain available offline, while attachment changes require a connection. Portable JSON backups include an attachment inventory but not the file bytes, so a complete server backup requires PostgreSQL and the attachment volume.
