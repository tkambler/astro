...
export async function pendingMutations(ownerId = owner()): Promise<NoteMutation[]> {
  ...
  return result.rows.map(row => ({ mutationId: row.mutation_id!, id: row.id,
    baseRevision: row.base_revision, title: row.title, body: row.body, tags: row.tags,
  ...
  await db.transaction(async tx => {
    ...
    if (local.dirty) {
      ...
      [crypto.randomUUID(), `${local.title.slice(0, 500 - suffix.length)}${suffix}`, local.body,
        local.created_at ?? local.updated_at, new Date().toISOString(), crypto.randomUUID(), ownerId, local.tags])
    }
    await tx.query(`UPDATE notes SET title=$1,body=$2,revision=$3,created_at=$4,updated_at=$5,deleted_at=$6,tags=$7::text[],
...
export async function receiveNote(note: Note, ownerId = owner()) {
  await ready()
  await db.query(`INSERT INTO notes (id,title,body,revision,created_at,updated_at,deleted_at,dirty,mutation_id,base_revision,owner_id,tags,synced_body,synced_title)
    VALUES ($1,$2,$3,$4,$5,$6,$7,false,NULL,$4,$8,$9::text[],$3,$2)
    ON CONFLICT (id) DO UPDATE SET title=$2,body=$3,revision=$4,created_at=$5,updated_at=$6,deleted_at=$7,tags=$9::text[],
    ...
    [note.id, note.title, note.body, note.revision, note.createdAt, note.updatedAt, note.deletedAt, ownerId, note.tags])
  announceChange()
}
